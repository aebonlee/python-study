          ...
          <div className="container">
            <div className="page-header-content">
              ...
              <div className="page-header-title-row">
                <div>
                  <h1>{localizedField(quiz, 'title')}</h1>
                ...
                <div key={quiz.id} className="quiz-card" onClick={() => setActiveQuiz(quiz.id)}>
                  ...
                  <div className="quiz-card-body">
                    ...
                    <div className="quiz-card-meta">
                      <span><i className="fa-solid fa-file-lines" /> {quiz.questions.length}{t('quiz.questions')}</span>
                      <span><i className="fa-solid fa-clock" /> {Math.floor(quiz.timeLimit / 60)}{t('quiz.minutes')}</span>
                      <span><i className="fa-solid fa-circle-check" /> {quiz.passingScore}{t('quiz.passingScore')}</span>
                    </div>
                    {bestScore !== undefined && (
                      <div className={`quiz-card-score ${passed ? 'passed' : 'failed'}`}>
                        {t('quiz.bestScore')}: {bestScore}{t('quiz.points')} {passed ? <><i className="fa-solid fa-check" /> {t('quiz.passed')}</> : <><i className="fa-solid fa-xmark" /> {t('quiz.failed')}</>}
                      </div>
                    )}
                  </div>
                  <div className="quiz-card-action">
                    <span className="btn btn-primary btn-sm">
                      {bestScore !== undefined ? t('quiz.retry') : t('quiz.start')}
                    </span>
                  </div>
                </div>
              )
      ...
      <section className="quiz-history-section">
        <div className="container">
          <h2 className="quiz-history-title">
            <i className="fa-solid fa-clipboard-list" /> {t('quiz.history')}
          </h2>
          <div className="quiz-history-table-wrap">
            <table className="quiz-history-table">
              <thead>
                <tr>
                  <th>{t('quiz.quizName')}</th>
                  <th>{t('quiz.attempt1')}</th>
                  <th>{t('quiz.attempt2')}</th>
                  <th>{t('quiz.attempt3')}</th>
                  <th>{t('quiz.finalStatus')}</th>
                  <th>{t('quiz.firstDate')}</th>
                  <th>{t('quiz.lastDate')}</th>
                </tr>
              </thead>
              <tbody>
                {quizList.map(quiz => {
                  ...
                  return (
                    <tr key={quiz.id}>
                      ...
                      <td className="quiz-status-cell">
                        {bestScore === undefined ? (
                          <span className="quiz-status not-taken">{t('quiz.notTaken')}</span>
                        ) : passed ? (
                          <span className="quiz-status pass"><i className="fa-solid fa-circle-check" /> {t('quiz.pass')}</span>
                        ) : (
                          <span className="quiz-status fail"><i className="fa-solid fa-circle-xmark" /> {t('quiz.fail')}</span>
                        )}
                      </td>
