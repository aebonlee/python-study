const e=[{name:"image",title:"이미지 변경 GUI",desc:"PhotoImage로 GIF 표시·교체",code:`# === 이미지 변경 GUI ===
...
window.mainloop()`},{name:"mypaint",title:"마우스 그림판",desc:"Canvas 드래그로 선 그리기",code:`# === 마우스 그림판 ===
# [주의] tkinter는 브라우저에서 실행할 수 없습니다.
# 코드를 다운로드하여 IDLE 또는 VS Code에서 실행하세요.
...
window.mainloop()`},{name:"mypaint2",title:"색상 버튼 그림판",desc:"색상 선택 버튼 추가 그림판",code:`# === 색상 버튼 그림판 ===
# [주의] tkinter는 브라우저에서 실행할 수 없습니다.
# 코드를 다운로드하여 IDLE 또는 VS Code에서 실행하세요.
...
window.mainloop()`},{name:"filename",title:"파일 대화상자",desc:"askopenfilename 파일 선택",code:`# === 파일 대화상자 ===
# [주의] tkinter는 브라우저에서 실행할 수 없습니다.
# 코드를 다운로드하여 IDLE 또는 VS Code에서 실행하세요.
...
if readFile != None:
    ...
    for line in infile.readlines():
        line = line.strip()
        print(line)
    infile.close()`},{name:"memo",title:"tkinter 메모장",desc:"열기·저장·종료 메뉴 메모장",code:`# === tkinter 메모장 ===
# [주의] tkinter는 브라우저에서 실행할 수 없습니다.
# 코드를 다운로드하여 IDLE 또는 VS Code에서 실행하세요.
...
window.mainloop()`},{name:"pillow5",title:"Pillow 이미지 편집기",desc:"회전·흐리기 등 영상처리 GUI",code:`# === Pillow 이미지 편집기 ===
...
window.mainloop()`},{name:"plot",title:"난수 선 그래프",desc:"matplotlib 기본 선 그래프",code:`# === 난수 선 그래프 ===
...
print("plt.show()")`},{name:"plot1",title:"요일별 기온 그래프",desc:"X·Y 데이터 선 그래프",code:`# === 요일별 기온 그래프 ===
# [주의] matplotlib는 브라우저에서 실행할 수 없습니다.
# pip install matplotlib 후 IDLE 또는 VS Code에서 실행하세요.
...
print("\\n[matplotlib 코드]")
print("plt.plot(X, Y)")
print("plt.show()")`},{name:"plot2",title:"두 도시 기온 비교",desc:"두 선을 하나의 그래프에",code:`# === 두 도시 기온 비교 ===
# [주의] matplotlib는 브라우저에서 실행할 수 없습니다.
...
print("plt.show()")`},{name:"plot3",title:"레전드·제목 그래프",desc:"label, legend, title 완성판",code:`# === 레전드·제목 그래프 ===
# [주의] matplotlib는 브라우저에서 실행할 수 없습니다.
...
print("plt.show()")`},{name:"graph",title:"2차 함수 그래프",desc:"ax²+bx+c 그래프 그리기",code:`# === 2차 함수 그래프 ===
# [주의] matplotlib는 브라우저에서 실행할 수 없습니다.
...
print("plt.show()")`},{name:"datediff",title:"날짜 차이 계산",desc:"datetime 모듈 활용",code:`# === 날짜 차이 계산 ===
...
print("시작일:", str(x))
print("현재:", str(y))
print("경과 일수:", str(delta.days) + "일")`},{name:"webopen",title:"웹브라우저 열기",desc:"webbrowser.open() 사용",code:`# === 웹브라우저 열기 ===
...
print("# webbrowser.open(full_url)  ← 데스크톱에서 실행하세요")`},{name:"webopen1",title:"구글 번역기 열기",desc:"URL 조합 후 브라우저 열기",code:`# === 구글 번역기 열기 ===
# [주의] webbrowser 모듈은 브라우저(Pyodide) 환경에서 실행할 수 없습니다.
...
print("# webbrowser.open(url)  ← 데스크톱에서 실행하세요")`},{name:"tts",title:"텍스트 음성 변환",desc:"gTTS로 mp3 변환·재생",code:`# === 텍스트 음성 변환 (TTS) ===
...
print("# os.system('output.mp3')  ← 생성된 mp3 재생")`}];export{e as default};
