const e=[{name:"forloop",title:"for 루프 기본",desc:"리스트 순회 반복 출력",code:`# === for 루프 기본 ===
# 리스트의 각 요소를 하나씩 꺼내 반복합니다.

for i in [1, 2, 3, 4, 5]:       # 리스트의 요소를 순회
    print("방문을 환영합니다.")   # 5번 출력됨`},{name:"forloop1",title:"for 루프 변수 출력",desc:"반복 변수 i 값 출력",code:`# === for 루프 변수 출력 ===
# 반복 변수 i가 리스트의 각 값을 순서대로 갖습니다.

for i in [1, 2, 3, 4, 5]:
    print("i=", i)   # i가 1, 2, 3, 4, 5로 변화`},{name:"forloop2",title:"9의 배수 출력",desc:"for 루프로 구구단 일부",code:`# === 9의 배수 출력 ===
# for 루프로 9×1부터 9×5까지 출력합니다.

for i in [1, 2, 3, 4, 5]:
    print("9*", i, "=", 9 * i)  # 9×i 계산 결과 출력`},{name:"range1",title:"range() 기본",desc:"range(5)로 5번 반복",code:`# === range() 기본 ===
# range(n)은 0부터 n-1까지의 숫자를 생성합니다.

for i in range(5):               # 0, 1, 2, 3, 4
    print("방문을 환영합니다!")    # 5번 반복 출력`},{name:"range",title:"range() 변형",desc:"시작·끝·증감 다양한 range()",code:`# === range() 변형 ===
...
for i in range(10, 7, -1):      # 10, 9, 8 (1씩 감소)
    print(f"변수 i의 값={i}")`},{name:"while1",title:"1~10 합 (while)",desc:"while 루프로 합산",code:`# === 1부터 10까지 합 (while) ===
...
print("합계는", sum)     # 1+2+...+10 = 55`},{name:"while2",title:"1~10 합 (while 변형)",desc:"동일 로직 while 변형",code:`# === 1부터 10까지 합 (while 변형) ===
...
print("합계는", sum)      # 결과: 55`},{name:"while3",title:"while로 사각형 그리기",desc:"while+turtle 사각형",code:`# === while로 사각형 그리기 ===
...
turtle.done()`},{name:"inf_loop",title:"무한 루프 + break",desc:"'blue' 입력 시 탈출",code:`# === 무한 루프 + break ===
...
print('전진!!')`},{name:"mtable",title:"구구단 출력",desc:"for range(1,10) 구구단",code:`# === 구구단 출력 ===
# 입력한 단의 구구단을 출력합니다.
...
for i in range(1, 10, 1):      # 1부터 9까지 반복
    print(f"{dan}*{i}={dan*i}")  # f-string으로 포맷팅`},{name:"mquiz",title:"랜덤 덧셈 퀴즈",desc:"while True 반복 퀴즈",code:`# === 랜덤 덧셈 퀴즈 ===
# 랜덤 덧셈 문제를 풀어봅니다.
...
for _ in range(3):              # 3문제 출제
    ...
    if answer == x + y:
        ...
        print("다음번에는 잘할 수 있죠?")`},{name:"login",title:"비밀번호 반복 입력",desc:"올바른 입력까지 while 반복",code:`# === 비밀번호 반복 입력 ===
...
for password in attempts:
    ...
    if password == correct_password:
        ...
        print("틀렸습니다. 다시 입력하세요.")`},{name:"guess",title:"숫자 맞추기 게임",desc:"1~100 높음/낮음 힌트",code:`# === 숫자 맞추기 게임 ===
...
while True:
    ...
    if guess < answer:
        ...
        break`},{name:"timer",title:"카운트다운 타이머",desc:"for+sleep 카운트다운",code:`# === 카운트다운 타이머 ===
...
print("시간 종료!")`},{name:"encrypt",title:"문자 코드 암호화",desc:"코드값 +1로 암호문 생성",code:`# === 문자 코드 암호화 (시저 암호) ===
...
print("평문:", plain_text)
print("암호문:", encrypted_text)`},{name:"decrypt",title:"문자 코드 복호화",desc:"코드값 -1로 평문 복원",code:`# === 문자 코드 복호화 ===
...
print("복호화:", plain_text)`},{name:"count",title:"모음 개수 세기",desc:"문자열에서 a,e,i,o,u 카운트",code:`# === 모음 개수 세기 ===
...
n = countVowel(s)
print(f"입력 문자열: {s}")
print(f"모음의 개수는 {n}개입니다.")`},{name:"remove_words",title:"단어 리스트 제거",desc:"for 루프로 특정 단어 제거",code:`# === 단어 리스트 제거 ===
# 문자열에서 특정 단어를 제거합니다.
...
print("삭제 후 남은 단어들:")
print(result)`},{name:"lotto",title:"로또 번호 생성기",desc:"중복 없이 6개 번호 뽑기",code:`# === 로또 번호 생성기 ===
...
print(f"생성된 로또번호: {getLotto()}")`}];export{e as default};
