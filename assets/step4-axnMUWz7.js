...
print(f"화씨 {ftemp}도 = 섭씨 {ctemp:.1f}도")`},{name:"int_sum",title:"정수 합 출력",desc:"두 정수를 입력받아 합 출력 (기본 I/O)",code:`# === 정수 합 출력 (기본 I/O) ===
...
print(f"{x} + {y} = {x + y}")`},{name:"bmi",title:"BMI 체질량 지수",desc:"키·몸무게로 BMI 계산 및 판정",code:`# === BMI 체질량 지수 계산기 ===
# Input: 키(cm), 몸무게(kg)
# Process: BMI = 몸무게 / (키m)²
...
print(f"BMI: {bmi:.1f}")
print(f"판정: {result}")`},{name:"discount",title:"할인 가격 계산기",desc:"정가·할인율로 최종 결제 금액 계산",code:`# === 할인 가격 계산기 ===
# Input: 정가, 할인율
...
print("=" * 30)`},{name:"time_calc",title:"초 → 시분초 변환",desc:"입력 초를 시·분·초로 분리 출력",code:`# === 초 → 시분초 변환기 ===
# Input: 총 초(seconds)
# Process: 시, 분, 초로 분리
# Output: 변환 결과 출력
...
print(f"변환: {hours}시간 {minutes}분 {seconds}초")`},{name:"score_avg",title:"시험 점수 평균",desc:"3과목 점수로 합계·평균·합격 판정",code:`# === 시험 점수 평균 계산기 ===
# Input: 3과목 점수
